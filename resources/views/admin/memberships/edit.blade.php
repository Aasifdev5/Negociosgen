                    ...
                    <form action="{{ route('memberships.update', $membership) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="price" class="form-control @error('price') is-invalid @enderror"
                                id="price" value="{{ old('price', $membership->price) }}">
                            @error('price')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div id="benefits-container">
                                @if (old('benefits', $membership->benefits ?? []))
                                    @foreach (old('benefits', $membership->benefits ?? []) as $index => $benefit)
                                        <div class="benefit-input">
                                            <input type="text" name="benefits[]" class="form-control mt-2"
                                                value="{{ $benefit }}">
                                        </div>
                                    @endforeach
                                @else
                                    <div class="benefit-input">
                                        <input type="text" name="benefits[]" class="form-control mt-2" value="">
                                    </div>
                                @endif
                            </div>
                            <button type="button" class="btn btn-secondary mt-2" id="add-benefit">Añadir beneficio</button>
                            <small class="text-muted">Escribe cada beneficio en un campo y haz clic en "Añadir beneficio"
                                para agregar más.</small>
                            @error('benefits')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="color" name="highlight_color"
                                class="form-control @error('highlight_color') is-invalid @enderror" id="highlight_color"
                                value="{{ old('highlight_color', $membership->highlight_color) }}">
