...
class SuccessController extends Controller
{
    ...
    public function index()
    {
        if (Session::has('LoggedIn')) {
            ...
            $data['SuccessTipss'] = SuccessTips::all();
            return view('admin.success.index', $data);
        }
    }
    ...
    public function create()
    {
        if (Session::has('LoggedIn')) {
            ...
            return view('admin.success.create', $data);
        }
    }
    ...
    public function store(Request $request)
{
    ...
    try {
        ...
        // Log the error
        Log::error('Error al crear SuccessTips: ' . $e->getMessage());
    ...
    public function edit($id)
    {
        if (Session::has('LoggedIn')) {
            ...
            $SuccessTips = SuccessTips::where('id', $id)->first();

            $data['SuccessTips'] = $SuccessTips;

            return view('admin.success.edit', $data);
        }
    }
    ...
    try {
        ...
        if ($request->hasFile('coach_thumbnail')) {
            ...
            if ($file && $file->isValid()) {
                ...
                $SuccessTips->coach_thumbnail = $this->uploadFile($file, 'coach_thumbnails');
            }
        }
        ...
        if ($request->has('coach_thumbnail_cropped')) {
            ...
            $SuccessTips->coach_thumbnail = 'uploads/coach_thumbnails/' . $fileName;
        }

        // Update the SuccessTips entry
        $SuccessTips->update([
            'coach_name' => $request->coach_name,
            'course_title' => $request->course_title,
            'course_description' => $request->course_description,
        ...
        Log::error('Error updating SuccessTips: ' . $e->getMessage());
...
public function uploadFile($file, $directory)
{
    ...
    public function bulkDelete(Request $request)
    {
        try {
            $request->validate([
                'ids' => 'required|array',
                'ids.*' => 'exists:SuccessTipss,id'
            ]);

            $SuccessTipss = SuccessTips::whereIn('id', $request->ids)->get();

            SuccessTips::whereIn('id', $request->ids)->delete();
